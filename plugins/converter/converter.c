...
static void
encoder_preset_free_data (ddb_encoder_preset_t *p) {
    if (p->title) {
        free (p->title);
    }
    if (p->extension) {
...
load_encoder_preset_dir (const char *path, const int builtin) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        struct encoder_preset_link *l = encoder_preset_load (s);
        if (l) {
